...
public sealed record IngestStats
{
    ...
    /// <summary>
    /// Move request lines the parser could not read at all. Anything above zero means the
    /// game changed its log format and item positions are going stale.
    /// </summary>
    public int UnparsedRequestLines { get; set; }

    /// <summary>
    /// Move request lines read only by their shape, under a tag the parser does not know.
    /// Nothing is lost, but the tag was renamed and the parser should learn the new name.
    /// </summary>
    public int FallbackRequestLines { get; set; }

    /// <summary>Which tags the two counters above came from, and how often.</summary>
...
public sealed class LogIngestor(TrackerDb db, string logDir, DateTimeOffset? fromDate = null)
{
    ...
    public void IngestFile(SqliteConnection cn, LogFile file, IngestStats stats)
    {
        ...
        SaveSession(
            cn, tx, session.Id, reader.Offset, firstTs, lastTs, state.CurrentLocation, complete: !file.IsLive);
        tx.Commit();
    ...
    /// <summary>
    /// Counts move request lines the parser missed, or only caught by their shape. The game
    /// renames tags and rewords lines between builds without notice, and a parser that stops
    /// matching fails silently — this is what makes it loud.
    /// </summary>
    private static void TrackDrift(LogLine log, InventoryEvent? ev, IngestStats stats)
    {
        if (InventoryEventParser.ReadRequestShape(log) is not { } shape) return;
        if (InventoryEventParser.IsNonRelocating(shape.MoveType)) return;

        if (ev is null)
        {
            // A Queued line with nothing in it is a multi-select drag's empty half; the
            // Add-move half that carries the batch is counted on its own.
            if (shape.IsDegenerate) return;
            stats.UnparsedRequestLines++;
        }
        else if (!InventoryEventParser.IsKnownRequestTag(log.EventName))
        {
            stats.FallbackRequestLines++;
        }
        else
        {
            return;
        ...
        stats.DriftTags[log.EventName] = stats.DriftTags.GetValueOrDefault(log.EventName) + 1;
    }
        ...
        using (var reset = cn.CreateCommand())
        {
            ...
            reset.CommandText = """
                ...
                SET byte_offset = 0, first_ts = NULL, last_ts = NULL, complete = 0,
                    cur_loc_id = NULL, cur_loc_since = NULL
                WHERE id = $s
                """;
