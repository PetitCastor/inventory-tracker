...
**Tag rename at build 12519617 (2026-08-26).** `<InventoryManagementRequest>` became `<Inventory Mgmt Request Queued>` (with `<Inventory Mgmt Request Handle>` for the "Attempting queuing" chatter and a new `<Inventory Mgmt Request Process>`), and the Add-move line went from `New request[` to `New Request[`. The rest of each line is byte-for-byte the same. The parser accepts both, and it reads any request body filed under an unknown tag by its shape. The ingestor counts every request line it misses, and the [reliability study](../reliability/README.md) fails if any are missed. Assume the game will do this again.
