...
public sealed class LogIngestorTests : IDisposable
{
    ...
    {
        new LogIngestor(_db, _dir).IngestAll(); // nothing yet
        Append(Arrive);
    ...
    [Fact]
    public void Counts_move_request_lines_it_cannot_read_and_those_it_reads_only_by_shape()
    {
        Append(
            // Readable, but under a tag the parser has never been told about.
            "<2026-08-27T00:46:52.940Z> [Notice] <Inventory Mgmt Request Enqueued> Queued Request[53] Type[Move] " +
            "for 'Pilot' [204821708183] Source Inventory[204821708183:Location:2273540638] " +
            "Target Inventory[757726468117:Container:0]. Source[behr_rifle_ballistic_03_tint03] amount[1] rank[a]. " +
            "Target[NULL] amount[0] rank[b]. Item[NONE] action[None]. RequestInProgress[0] CurrentProcess[]",
            // Reworded past what the patterns follow.
            "<2026-08-27T00:47:00.000Z> [Notice] <Inventory Mgmt Request Queued> Queued Request[54] Type[Move] " +
            "for 'Pilot' [204821708183] FromInventory[1:Location:2] ToInventory[3:Container:0].",
            // Reads nothing, relocates nothing: not drift.
            "<2026-08-27T00:47:01.000Z> [Notice] <Inventory Mgmt Request Queued> Queued Request[55] Type[QueryInventory] " +
            "for 'Pilot' [204821708183] Source Inventory[INVALID] Target Inventory[INVALID].");

        var stats = new LogIngestor(_db, _dir).IngestAll();

        Assert.Equal(1, stats.MovesInserted);
        Assert.Equal(1, stats.FallbackRequestLines);
        Assert.Equal(1, stats.UnparsedRequestLines);
        Assert.Equal(1, stats.DriftTags["Inventory Mgmt Request Enqueued"]);
        Assert.Equal(1, stats.DriftTags["Inventory Mgmt Request Queued"]);
    }
    ...
    public void A_parser_upgrade_re_ingests_history_that_was_read_by_the_old_one()
    {
        ...
        new LogIngestor(_db, _dir).IngestAll();
        Assert.Equal("1", Scalar("SELECT CAST(COUNT(*) AS TEXT) FROM move"));

        using (var cn = _db.Open())
        using (var cmd = cn.CreateCommand())
        {
            cmd.CommandText = "UPDATE meta SET value = '1' WHERE key = 'parser_version'";
            cmd.ExecuteNonQuery();
        }

        _db.Initialize();

        Assert.Equal("0", Scalar("SELECT CAST(COUNT(*) AS TEXT) FROM session"));
        Assert.Equal("0", Scalar("SELECT CAST(COUNT(*) AS TEXT) FROM move"));
    }
