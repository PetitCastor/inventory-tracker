...
public static class ReliabilityStudy
{
    ...
    public static ReliabilityReport Run(string corpus, string logDir, string workDir, Options? options = null)
    {
        ...
        foreach (var (tag, n) in stats.DriftTags) report.DriftTags[tag] = n;

        report.Metrics["capture.unparsed_lines"] = stats.UnparsedRequestLines;
        report.Metrics["capture.fallback_lines"] = stats.FallbackRequestLines;

        MeasureCapture(db, files, report);
        MeasureEnrichment(db, report);

        var asOf = EndOfCorpus(db);
        var resolver = HoldingResolver.Load(db, asOf);
    ...
    private static void MeasureLedger(HoldingResolver resolver, ReliabilityReport report)
    {
        ...
        var applied = s.NamedMoves + s.ClassMoves + s.BackpackSkipped;

        report.Metrics["ledger.source_hit_rate"] = s.SourceHitRate;
        report.Metrics["ledger.backpack_skip_share"] = applied == 0 ? 0 : (double)s.BackpackSkipped / applied;
        ...
        report.Counts["ledger.backpack_skipped"] = s.BackpackSkipped;
    ...
    private static HashSet<string> Fingerprint(TrackerDb db)
    {
        ...
        using (var cmd = cn.CreateCommand())
        {
            ...
            while (r.Read())
            {
                var row = string.Join('|', Enumerable.Range(0, r.FieldCount).Select(i => Convert.ToString(r.GetValue(i), CultureInfo.InvariantCulture)));
